...
import copy
import random
from misc import get_weather
from const import questions

app = Flask(__name__)
app.secret_key = 'FPHNAIHDaıshdDHshdPASahdı565ADAs'
...
def index():
    ...
    return render_template('index.html', logged_in='username' in session, weather_data=weather_data,
                        processed_weather_data=processed_weather_data)
...
def quiz():
    if 'username' not in session:
        flash('Sınavı yapabilmek için giriş yapınız!','default')
        return redirect("/")
    username = session.get('username')
    user = User.query.filter_by(username=username).first()

    if user and user.score is not None:
        flash('Sınavı tekrar yapamazsınız!','danger')
        flash('Skor: ' + str(user.score),'default')
        return redirect("/")
    ...
    if session['asked'] < len(questions):
        ...
        if user:
            ...
            if score < 60:
                ...
                session["asked"] += 1
                session['questions'].remove(current_question)
                flash('Sınavı geçmek için en az 60 puan almanız gerekmektedir.','danger')
                return render_template('quiz.html' ,show_question=True, score=score, question=current_question, logged_in=True)
